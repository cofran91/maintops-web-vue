...
export const ownersApi = {
  async index(params = {}) {
    ...
    return normalizePaginatedData(
      unwrapApiData(response.data, 'Owners could not be loaded.'),
    )
  },
  ...
  async show(id) {
    ...
    return unwrapApiData(response.data, 'The selected owner could not be loaded.')
  },

  async create(payload) {
    const response = await http.post(ENDPOINT, payload)

    return unwrapApiData(response.data, 'The owner could not be created.')
  },

  async update(id, payload) {
    const response = await http.patch(`${ENDPOINT}/${id}`, payload)

    return unwrapApiData(response.data, 'The owner could not be updated.')
  },
