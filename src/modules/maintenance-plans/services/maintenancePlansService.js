...
export const maintenancePlansApi = {
  async index(params = {}) {
    ...
    return normalizePaginatedData(
      unwrapApiData(response.data, t('maintenancePlans.errors.load')),
    )
  },
  ...
  async show(id) {
    ...
    return unwrapApiData(response.data, t('maintenancePlans.errors.show'))
  },

  async create(payload) {
    const response = await http.post(ENDPOINT, payload)

    return unwrapApiData(response.data, t('maintenancePlans.errors.create'))
  },

  async update(id, payload) {
    const response = await http.patch(`${ENDPOINT}/${id}`, payload)

    return unwrapApiData(response.data, t('maintenancePlans.errors.update'))
  },
